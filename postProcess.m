function [] = postProcess(FileName,data_dir,save_dir)
...
%   C - image that centerline voxels have the value equal to the vessel ID    
%
% Example
% ---------
...
deadEndLimit = 5; % to remove dead end hairs
diameterLimit = 200; %25; % to remove large vessels
...
%    A = single((A + A') > 0);
%    BGobj = digraph(A);
%    dist = distances(BGobj);
%    dist(isinf(dist)) = 0;
%    [~, j] = max(dist(:));
%    [S, T] = ind2sub(size(A), j(1));
%    [~, path, ~] = distances(BGobj, S, T);
%    if ~isempty(path)
%        Skel{1, i} = temp(path, :);
%    end
%end
